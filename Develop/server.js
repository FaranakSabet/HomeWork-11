...
app.get("/api/notes", function(req,res){
 ...
 app.get("/api/notes/:id", function(err, res){
     notes = JSON.parse(fs.readFile(path.join(__dirnamem "/db/db.json"), "utf8"), function(err, data){
         if (err) throw err;
     });
  ...
  app.post("/api/notes", function(req,res){
      notes = JSON.parse(fs.readFileSunc(path.join(__dirname, "/db/db.json"), "utf8"));
      newNote = req.body;
  ...
  app.delete("/api/notes/:id", function(req,res){
      ...
      fs.writFileSync(path.join(__dirname, "/db/db.json"), JSON.stringify(notes), "utf8", function(err){
          if(err) throw err;
